...
int edn_init(edn_model *m, const void *source, size_t length) {
    ...
    if (!m || !data || length < EDN_HEADER_BYTES ||
        ...
        read_u32(data + 24) != length || (data[23] != 1 && data[23] != 2)) return -1;
    ...
    if (!region_valid(dsp_offset, data[23] == 1 ? 3988u : 3992u, length) ||
        dsp_offset + (data[23] == 1 ? 3988u : 3992u) != length ||
        memcmp(data + dsp_offset, data[23] == 1 ? "DSP1" : "DSP2", 4) || read_u16(data + dsp_offset + 4) != 16000 ||
    ...
    if (data[23] == 2) {
        /* Positive IEEE754 float32 encodings are ordered as unsigned integers.
         * Validate (0,8] without introducing float arithmetic into this core. */
        uint32_t gain = read_u32(data + dsp_offset + 3988u);
        if (!gain || gain > UINT32_C(0x41000000)) return -1;
    }
