...
int edn_audio_init(edn_audio_state *s, const edn_model *m, void *neural_state, size_t state_length) {
    const uint8_t *p;
    unsigned i;
    if (!s || !m || !m->dsp_data || (m->dsp_bytes != 3988 && m->dsp_bytes != 3992) ||
    ...
    s->output_gain = m->dsp_bytes == 3992 ? edn_dsp_read_f32(m->dsp_data + 3988) : 1.0f;
    if (!isfinite(s->output_gain) || s->output_gain <= 0 || s->output_gain > 8) return -1;
    p = m->dsp_data + 20;
    for (i = 0; i < 512; ++i) {
...
int edn_audio_process(edn_audio_state *s, const float input[256], float output[256]) {
    ...
    if (edn_dsp_synthesize(s->fft, s->window, s->synthesis, s->synthesis_weight,
                          s->deltas, s->model->output_exponent, s->mask_scale, output)) return -1;
    if (s->output_gain != 1.0f)
        for (i = 0; i < 256; ++i) output[i] *= s->output_gain;
    return 0;
}
