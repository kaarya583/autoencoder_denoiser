...
m=build_model('gtcrn',{}).cuda().train();o=torch.optim.AdamW(m.parameters(),lr=.001)
...
r=pathlib.Path('/content/esp32_runs');(r/'gtcrn_cuda_pilot.json').write_text(json.dumps({'steps':8,'losses':losses,'finite_gradients':True,'elapsed_seconds':time.monotonic()-t,'stats':m.model_stats()},indent=2));print('PILOT_PASS',losses,flush=True)
for n in ['esp32_gtcrn_float','esp32_gtcrn_broad_float']:
...
log=open(r/'float_gtcrn_broad_external_watch.log','w');p=subprocess.Popen([sys.executable,'-m','esp32_denoiser.development_checkpoints','--run',str(r/'float_gtcrn_broad'),'--manifest','/content/extra_audio/development/mixtures.jsonl','--device','cuda','--every-epochs','5','--max-hours','7'],stdout=log,stderr=subprocess.STDOUT,start_new_session=True);print('GTCRN_WATCH',p.pid,flush=True)
